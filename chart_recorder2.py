...
#sdh
    # load steady data filter
    private.FilterData = DataSteady()
    private.n = 10  #the data array block is Npmax=1e4 times this!
    # setup the plot
    data_chart.curves_per_plot = [1]
    data_chart.set_plot_properties(0, title='Recorded Voltages', x_label='time (s)', y_label=u'$V_1$ (V)')
...
def run(time_display, voltage_1_display, data_chart, dwell_time, 
        start_stop, error_dialog, open_close, yset, **kwargs):
    try:
        ...
        t0 = time.time()
        t_of_packet = dwell_time.value/n *np.linspace(0,n-1,n)
        
# get first data block
        new_y1 = np.random.normal(size=n)
        new_t = t0 + t_of_packet 
        new_data = np.array([new_t, new_y1])
        private.FilterData.Clear(new_y1)
       
        while(start_stop.value):
            private.logger.debug('acquiring data')
            # read and show voltages
            new_t = time.time() - t0 + t_of_packet
            time_display.value = new_t
            new_y1 = np.random.normal(loc=yset.value,size=n)
            voltage_1_display.value = new_y1
            y1 = new_y1
            private.data.append(y1)
            new_data = np.array([new_t, new_y1])
